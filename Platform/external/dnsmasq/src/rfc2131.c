...
static int sendEventToNetdSocket(int argc, char argv[][100]) {
    ...
    if ((sock = socket_local_client(argv[1],
        ...
        if ((sock = socket_local_client("netd",
                                         ANDROID_SOCKET_NAMESPACE_RESERVED,
                                         SOCK_STREAM)) < 0) {
            fprintf(stderr, "Error netd connecting (%s)\n", strerror(errno));
            return -1;
        }
        my_syslog(MS_DHCP | LOG_INFO, "Failed Error connecting to create %s", argv[1]);
...
static int do_cmd(int sock, int argc, char argv[][100]) {
    ...
    my_syslog(MS_DHCP | LOG_INFO, "Final command %s", final_cmd);
