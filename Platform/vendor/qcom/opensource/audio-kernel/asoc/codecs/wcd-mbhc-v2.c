				...
				enum snd_jack_types jack_type)
{
	...
	WCD_MBHC_RSC_ASSERT_LOCKED(mbhc);
//bug 492687, baoshulin@wingtech.com, 20191016, modify for car AUX cable function
//ExbB P200624-06568 baoshulin@wingtech.com, 20190629, modify remove lineout
	...
	printk("%s:audiock_mbhc insertion %d, jack_type %x, hph_status %x, RL %d ohm, RR %d\n",
...
//ExbB P200624-06568 baoshulin@wingtech.com, 20190629, modify clear all jack snd
        pr_info("audiock report all 0 \n");
		wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADSET);
		wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADPHONE);
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	printk("%s: audiock_mbhc button press %x\n", __func__, mbhc->buttons_pressed);
done:
	pr_debug("%s: leave\n", __func__);
	WCD_MBHC_RSC_UNLOCK(mbhc);
	return IRQ_HANDLED;
