...
//bug 535039, baoshulin@wingtech.com, 20200228, modify for audio bringup,modify headset ref voltage
 ...
 */
//bug 537023, baoshulin@wingtech.com, 20200316,  modify for headset volume up key
static struct wcd_mbhc_config wcd_mbhc_cfg = {
	...
	.key_code[6] = 0,
	.key_code[7] = 0,
//bug 535039, baoshulin@wingtech.com, 20200228, modify for audio bringup, Car AUX cable function
	.linein_th = MAX_IMPED,
	.moisture_en = false,
...
//ExtB P200714-05807 audio- update headset-mic to headphone pcmid
//bug 559347 ,baoshulin,20200709,loopback-hsmic-headphone for audio ata
	{/* hw:x,39 */
...
/*+Bug 538153 baoshulin.wt modify for compatiable smartpa*/
    pr_info("%s : smartpa_type is :%d\n",__func__, smartpa_type);

    if(FS18XX==smartpa_type){
        msm_mi2s_be_dai_links[0].codec_dai_name="fs18xx-aif";
        msm_mi2s_be_dai_links[0].codec_name="fs18xx.0-0036";
        dev_info(dev, "%s: audiock kernel pa dai config name:%s \n",__func__,msm_mi2s_be_dai_links[0].codec_dai_name);
    }
/*-Bug 515704 baoshulin.wt modify for compatiable smartpa*/
	...
	if (!strcmp(match->data, "codec")) {
		...
		if (rc) {
			...
			if (mi2s_audio_intf) {
				...
				dev_err(dev, "%s:audiock_machine load fsm dai link[0]codec_dai_name:%s codec_name:%s\n",
				__func__,msm_mi2s_be_dai_links[0].codec_dai_name,msm_mi2s_be_dai_links[0].codec_name);
				total_links +=
					ARRAY_SIZE(msm_mi2s_be_dai_links);
			}
		}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: audiock kernel Registration asoc machine probe start\n",__func__);
	...
	dev_info(&pdev->dev, "%s: audiock kernel card name:%s\n", __func__, &card->name);
	...
	dev_info(&pdev->dev, "%s: audiock kernel Registration asoc dai_link ok\n",__func__);
	...
	dev_info(&pdev->dev, "%s: audiock kernel Sound card %s registered\n",
		 __func__, card->name);
