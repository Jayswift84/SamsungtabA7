...
//ExtB P200611-04458 ,baoshulin.wt,add,20200617,modify not found soundcard
	...
	if (swrm->version >= SWRM_VERSION_1_6) {
		if (swrm->swrm_hctl_reg) {
			...
			usleep_range(500, 505);
			dev_err(swrm->dev, "TEST: hctl_reg val: 0x%x\n", ioread32(swrm->swrm_hctl_reg));
		}
	}
...
//ExtB P200611-04458 ,baoshulin.wt,add,20200617,modify not found soundcard
		for (i = 0; i < len; i++) {
			usleep_range(50, 55);
			dev_err(swrm->dev,
				"TEST:reg:0x%x val:0x%x\n",
				reg[i], swr_master_read(swrm, reg[i]));
		}
		dev_err(swrm->dev,"TEST: return -EINVAL,ignore..\n");
		//return -EINVAL;
	}
	/*
